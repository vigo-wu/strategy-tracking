# coding: utf-8
"""将 qmt_terminal_trend_pb_basket.py 部署为国金 QMT 模型交易用的 GBK 文件。

编辑 scripts/qmt/qmt_terminal_trend_pb_basket.py 后运行本脚本。
"""
...
SRC = HERE / "qmt_terminal_trend_pb_basket.py"

QMT_DIR = Path(r"D:\service\GJQMT") / "python"
TARGETS = [
    QMT_DIR / "TrendPBBasket.py",
    QMT_DIR / "趋势回调池.py",
]
...
def main() -> None:
    ...
    compile(data, "TrendPBBasket.py", "exec")
    for dest in TARGETS:
        try:
            ...
            print("SKIP", dest, e)
    print("OK: 打开国金 QMT -> 模型交易 -> 加载 TrendPBBasket / 趋势回调池")
    print("主图: 000825.SH | 周期=日线 | 部署后请重新编译")
