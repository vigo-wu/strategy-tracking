...
8. **UI 后台进程**：`Popen` `robust_run.py`，读 `progress.json`。caption 含路数/阶段；参数指纹预检与启池先写盘。暂停须杀进程树，再用操作系统命令行双重核对 pid，确认退出后再处理目录；**已完成组留盘**，禁止套用网格 dirty 删目录（残留进程未退出时删目录会误删正在写入的产物）。继续靠成交表缓存跳过已完成组（须有与 log 同 stem 的 `.ovfp` 指纹匹配）；**overrides / `compound_backtest` 变了自动失效重跑**，无需手删目录。换参后须开跑，不能只点「只汇总」（汇总仍读盘上旧成交表）。
