---
name: qmt-fband-extend
description: >-
  在 factor_band 增减技术指标或登记/启用/卸下/除名因子：指标走 indicators/ + ctx + _MODULE_HEAD；
  因子登记走 catalog.LEAVES + lib/<id>.py，启用走四个槽位的条件抽象语法树。Use when the user mentions
  加因子、删因子、登记、启用、卸下、除名、加减指标、新指标、catalog、LEAVES、
  _factor_eval_、indicators/、MODULE_HEAD、指标周期窗，或要把一条规则做成 FactorBand 因子。
---
...
| 词 | 是 | 不是 |
| :--- | :--- | :--- |
| **指标** | 纯数学序列计算（序列→序列或纯工具）；不带多空立场，不读 `RECIPE` / 配置 | 叶子、因子 |
| **因子** | 返回 `(bool, detail)` 的原子信号；`_factor_eval_<id>`；`lib/<id>.py` | 买卖立场；仓位门槛；指标 |
| **叶子** | 买入 `entry`、加仓 `scale_in`、卖出 `exit`、减仓 `scale_out` 四个槽位条件树中的引用代号；reasons 用这个 id | 指标；「在 `LEAVES` 里」本身（有行 ≠ 已启用） |
...
1. **`LEAVES`**：无阈值因子（如 `above_ma`）`params` 留空，**不要**写出空 `{}` 进 `factor_params`。网格轴序用 `params[].axis`。`default` 类型须稳定，见 [reference-catalog.md](reference-catalog.md) 的类型安全防错警告。
