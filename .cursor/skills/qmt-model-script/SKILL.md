...
description: >-
  ...
  account is not defined、get_history_data、back test mode,
  or 把策略转成 QMT 脚本.
---
...
| 终端报错 / 回测无日志 | **C. 排障** → [reference-pitfalls.md](reference-pitfalls.md) |
| 策略变更后同步脚本 | **B. 对齐** |
| 部署到 `QMT\python\` | `python scripts/_deploy_qmt_gbk.py` |
...
- [ ] 7. _deploy_qmt_gbk.py 写入终端 python\；model.md 写操作步骤
- [ ] 8. 回测先见 diag: ok，再改 DRY_RUN=False 实盘
```
...
- [ ] 2. GBK 部署并重新编译
- [ ] 3. 看日志：BACKTEST / DRY_RUN / diag ok|empty|short
- [ ] 4. 主图=标的、日线、区间足够长
- [ ] 5. 修源码 → _deploy_qmt_gbk.py → QMT 再编译
```
...
| 路径 | 完成标准 |
| :--- | :--- |
| **终端转写** | 有 `qmt_terminal_*.py` + deploy；回测能出 `diag: ok` 与信号行；model 有步骤 |
| **xtquant 转写** | 有 `qmt_*.py`；DRY_RUN 默认；model 标明外部运行 |
| **排障** | 对应该节陷阱已消除；用户日志可解释 |

## 附加资源

- [reference-pitfalls.md](reference-pitfalls.md) — **国金终端联调踩坑（优先读）**
