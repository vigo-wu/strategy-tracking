...
**禁止**把 MAE 反事实或 DSR/PBO 当作选参结论。 **禁止**为扫参改主题 `config.py`。
...
6. **时空双重隔离**：时间用 `tune_*` / `check_*`；空间用 `tune_stocks` / `holdout_stocks`（空间分进入排名，盲测不再只否决）。`mode=off` 时无空间维，20% 权重按 30:25:25 摊到另三维。
...
9. 每格先跑参数指纹预检（Dummy Context Check，不回放 K 线）必核 `recipe=`。人读 init 字段是启用叶子的点路径（`atr_stop.k=`、`above_ma.n=`）。该格 `overrides.factor_params` 且叶子已启用时再核对应路径（`trail_stop.tiers` 用 compact JSON）。卸下叶子的覆盖只走 `recipe=` 全表哈希。启用集合读不到则字段级跳过，只核 `recipe=`。不一致则停。通过后该格全部 walk 再跑。
...
取消硬门否决。全部格子按四维综合分排序，总分最高即推荐（接近 1.0 分则少改与 config 的差异键）。仅全部无法打分才无推荐（不要写「最优」或「维持现行」）。

**统计口径。** 几何年化 \((E_{end}/E_{start})^{1/n}-1\)，\(n=\) 窗内日历年数，**空仓年计入分母**。路径回撤；卡玛 = 几何年化 / `|max_dd|`（展示用，不排序）；账户盈亏 \(E_{end}-E_{start}\)。权益 = 预算 + 已实现盈亏台阶（与实盘评估相同，不承诺全日盯市）。验收窗接在调参权益之后（同一 walk 切开）。禁止用样本级整段 `max_dd`/`win_rate`。
...
- 防御：同账户 `all`/`tune`/`check` 最坏 `|max_dd|`，平方回撤（0 分线默认 35%）；触顶则总分 ×0.2。调参/验收（有盲测再加盲测验收）账户盈亏每有一个 ≤0 扣 15，下限 0。盲测篮回撤不混入防御。
- 结构：只用验收期。`Factor=(胜率%/100)×min(盈亏比,封顶)`，默认封顶 5、目标 0.5；笔数低于 0 分线得 0，默认 `30–80` 从 15 线性到 30，满分线及以上得 30。
- 复原：调参/验收夏普按目标（默认 0.5）归一后平均 ×50；年化衰减 Hermite（`r≥1→50`，否则 `50×(3r²−2r³)`），任一窗年化为负得 0。
- 泛化：双正线性比；盲测 ≤0 或缺窗得 0；调参 ≤0 且盲测 >0 走情况 C（`min(80, 50×盲测/sharpe_target)`），取消旧「满分 100」。

侧栏 / spec `score` 是评分维配置（权重、回撤 0 分线、获利因子目标、盈亏比封顶、夏普目标、笔数 0 分/满分线）。改旋钮再点「只汇总」按 `score` 重算推荐并写入 `summary.json`；界面主表用侧栏配置现算。`--score-json` 覆盖只汇总。`--gate-json` 已忽略（WARN）。公式形状（平方回撤、触顶×0.2、Hermite 衰减、情况 C 封顶 80、盈亏盾 30/扣 15、结构 70/30、复原 50/50、接近垫 1.0）写死在 `scripts/grid_score.py`。
...
- [ ] 3. 若空间隔离：freeze 含 tune/holdout；空间分进入排名
- [ ] 4. 运行时 overrides（禁止改 config 扫参）
- [ ] 5. 隔离 report/grid/<sweep>/；全局单层多进程池（禁止嵌套）
- [ ] 6. 参数指纹预检与格子一致（只跑 init，不走 K 线）
- [ ] 7. summarize → summary.json；四维综合分推荐
- [ ] 8. 一句综合推荐 + 总分；默认不改 config / 不 deploy
```

## 怎么跑（factor_band 首个实现）

仓库根目录。其它主题未接 `scripts/local_bt/` 前不要把路径写死成唯一实现。

```bash
python factor_band/scripts/local_bt/grid_run.py --spec .cursor/skills/qmt-local-bt-grid/examples/stop_loss.json
python factor_band/scripts/local_bt/grid_run.py --spec path/to/cells.json --batch-size 10
python factor_band/scripts/local_bt/grid_run.py --resume --sweep-dir factor_band/report/grid/<sweep>
python factor_band/scripts/local_bt/grid_run.py --spec .cursor/skills/qmt-local-bt-grid/examples/stop_loss_space.json --reshuffle
python .cursor/skills/qmt-local-bt-grid/scripts/summarize.py --sweep-dir factor_band/report/grid/<sweep>
```

`grid_run.py` 结束时会调 summarize，写出 `factor_band/report/grid/<sweep>/summary.json`（`report/` 已 gitignore）。

| 参数 | 含义 |
| :--- | :--- |
| `--spec` | 命名格子 JSON/YAML |
| `--workers` | 全局并行进程数（0=自动=min(walk数, CPU)；1=串行；格间与格内 walk 共用全局单层池） |
| `--batch-size` | 每组格子数；0=一组跑完全部（CLI 默认）。未完成组整组重来 |
| `--resume` | 同一 sweep 续跑：跳过 done，dirty 整组重跑；读磁盘 spec/freeze（含 `score`），忽略侧栏年份 |
...
| `--score-json` | 覆盖评分配置，只汇总重算推荐 |
| `--gate-json` | 已忽略（WARN），请用 `--score-json` |
