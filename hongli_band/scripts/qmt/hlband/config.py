...
#   价格贴近中/慢均线（|价-均线|/均线 <= 容差）且连续 N 日量 < 当日均量 * 比例
#   贴均线只看当天；缩量按 VOL_PULLBACK_CONFIRM_DAYS 连续确认
#   中线关闭时只贴慢线（当前 D_MA_MID=0，只贴 MA60）
MA_TOUCH_TOL = 0.015          # 0.015 = 距均线 ±1.5% 内算「回踩到位」
...
#   收盘跌破中线且量 < N 日均量 * 比例 → 当天任何买点失效
#   中线关闭时本条关掉（当前 D_MA_MID=0）
VOL_DRY_N = 20
