...
- **日度仓位**（组合明细面板）：由成交轮次推导，持仓区间 `[买入日, 卖出日)`；槽位=当日重叠 lot 数；资金占用率=`Σ成本/当前权益`（当前权益=预算+已实现盈亏阶梯，非市值）。主图双轴阶梯 + 按票成本堆叠；副图为按票持仓天数条形与槽位占用直方。主图框选日期（或手改日期窗）同步重算副图/KPI；点条形或下拉可高亮该票堆叠。
