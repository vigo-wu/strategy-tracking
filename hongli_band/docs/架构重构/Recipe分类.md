...
配套：[架构重构/架构.md](架构.md)（因子库目录）、[架构优化.md](./架构优化.md)、[草图.png](./草图.png)。
...
引擎可支持细原子，但默认 Recipe **只引用**下表。每个 id 对应 `hlband/factors/lib/<id>.py`。内部已复合的条件仍由该因子自己的参数（含 `D_MA_*<=0` 关条）处理。
