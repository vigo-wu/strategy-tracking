...
**主题目录**：`hongli_band/`｜**版本**：v1.7｜**运行**：国金 QMT 终端模型（见 §5）
...
（v1.7 用 MA60 弹性缓冲替代 v1.6「>20 日且浮盈&lt;2.5%」无条件平仓。）
...
日志确认 `HlBand v1.7 init` 后再改 `DRY_RUN=False`。只改 `hlband/*.py` 后 re-deploy，勿手改终端 GBK。
