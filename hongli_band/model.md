...
**主题目录**：`hongli_band/`｜**版本**：v1.13｜**运行**：国金 QMT 终端模型（见 §5）
...
| 卖点 | 条件 | 日志码 |
| :--- | :--- | :--- |
| ① 移动止盈 | ATR 阶梯融合：浮盈未达 3% 不激活；3%~6% 用 `1.0×ATR%` 夹逼 [1%,1.5%]；6%~12% 用 `1.5×ATR%` 夹逼 [2%,3.5%]；≥12% 用 `2.0×ATR%` 夹逼 [2%,3%]；自峰值回撤超阈值 → 平仓 | `trail_stop` |
...
| `TRAIL_ACTIVATE/MID/HIGH` | `0.03` / `0.06` / `0.12` | 阶梯浮盈门槛 |
| `TRAIL_S1/S2/S3_*` | 见 config | 各档 ATR 倍数与夹逼 |
| `TRAIL_ATR_PERIOD` | `14` | ATR 周期 |
...
日志确认 `HlBand v1.13 init`（`trail=fusion/ATR14`）后再改 `DRY_RUN=False`。
