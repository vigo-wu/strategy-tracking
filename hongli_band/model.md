...
**主题目录**：`hongli_band/`｜**版本**：v1.33｜**形态**：单仓骨架 / 分笔多仓｜**运行**：国金 QMT 终端模型（见 §5）  
...
空仓时开第一笔（`pullback_vol`）。已持仓且门槛+触发都满足时挂 `pending_entry add=True`，尾盘再买一笔预算（错过则次日开盘补）。加仓仍受下方全局拦截（破平台/金叉不受 `chase_skip`）；另有 `scale_bars` / `scale_w_hist` / `scale_sell_block`。周线空头 / 乖离 / 斜率 / 无量阴跌会取消加仓 pending。
