...
**Quant Station** 用 **KlineDump 日线 CSV** 回放 **真实拼接的 HlBand 脚本**（`qmt_common` + `hlband` 片段），不经过国金编辑器回测。买卖规则、均线、复权键仍以 [`model.md`](./model.md) 与 [`scripts/qmt/fband/config.py`](./scripts/qmt/fband/config.py) 为准。
...
1. Python 能 `import pandas`；界面还需要 Streamlit / Plotly：
   ...
   ```bash
   pip install -r factor_band/scripts/local_bt/requirements.txt
   ```
...
2. 仓库里有日线 CSV。默认根目录：`tools/csv/`。每种复权一个子目录：
   ...
   **时点前复权（PIT）**：勾选 `front` / `front_ratio` 时，回放读 `csv/none` + `csv/divid_factors/`。`front_ratio`→等比 `Πdr`（`mode=ratio`）；`front`→价差事件序（`mode=diff`）。报告仍写到 `report/front*`，日志含 `pit=1 mode=…`。两条结果应不同。缺 none 或因子 JSON → 硬失败。`DUMP_STOCKS` 须覆盖要测的票；`DIVIDEND_TYPES` 须含 `none`。K 线对 `front` / `front_ratio` 同样用 none 原料，按可见区间最后一根 bar 做 PIT，不画 none 原价，也不读静态 `csv/front*`。图表价差 PIT 配股按全认购；持仓除权仍默认未认购。成交轮次认操作明细「送转」行；持有回撤在 PIT 下按 none 收盘 × 累计送转股数（市值口径），避免除权跳空当成 −50% 回撤。
...
等价于 `streamlit run factor_band/scripts/local_bt/app.py`。浏览器打开后，顶栏选模式。

成功标志：能列出 `csv/<type>/` 里的标的，或「仅分析」能看到已有操作明细。

界面表格统一 **中文表头**；凡含标的代码的列，列名用「代码」，并在旁紧跟「名称」（来自 `stock_meta.json` / `stock_meta()`，未知则显示裸代码）。磁盘 CSV schema 不变。全市场名单可用 `python factor_band/scripts/local_bt/fetch_stock_meta.py` 从交易所刷新。
...
factor_band/report/<type>/ ← 本地回测 log / 操作明细 / 批量汇总
factor_band/report/local_bt_stock_select.csv  ← 选股总表（写在 report 根，不进某个 <type>）
factor_band/回测记录/      ← 旧终端导出；「仅分析」会一并列出
```
...
扫描侧栏勾选的 `report/<type>/` **并集**，外加 `factor_band/回测记录`。用来看已有成交表、权益、K 线，不再开跑。

### 选股方案

扫描已有分年回测后打分，给出 `BOOK_STOCKS` 草稿。不自动改 config。
...
产物：`factor_band/report/local_bt_stock_select.csv`。刷新缓存按钮会清扫描缓存。
...
```bash
python factor_band/scripts/local_bt/select_analysis.py \
...
起止年与换仓周期在表单外，改完立刻刷新段数。常量默认值见 `factor_band/scripts/local_bt/select_config.py` 的 `ANALYSIS_*`；组合仓位默认回落 `hlband/config.py`。

#### 产物与结果字段

**总表**（固定路径）：`factor_band/report/local_bt_select_analysis.csv`
...
```bash
python factor_band/scripts/local_bt/stock_select.py --report-dir factor_band/report --csv-dir tools/csv
python factor_band/scripts/local_bt/stock_select.py --report-dir factor_band/report --csv-dir tools/csv --year-start 2024 --year-end 2025
```

默认写出 `factor_band/report/local_bt_stock_select.csv`。`--year-start` / `--year-end` 含端点；不传则用扫描到的全部自然年。建议均线、建议复权与硬过滤都在该窗口内重算。`--min-n-buy-year` 为每年最少轮次（0 不启用）。
...
- **改了 `hlband/*.py` 却去改终端 GBK。** 本地回测每次拼接片段；终端模型仍须 `python factor_band/scripts/qmt/_deploy_qmt_gbk.py`。
- **选股 snippet 会写入 config。** 页面草稿不会自动改 config；要改跟踪池须自己编辑 `BOOK_STOCKS` 再 deploy。不要把最后一年 Top N 写成静态池。
...
- 终端回测 log → 图与 Markdown：仓库 skill `qmt-backtest-report`（`python factor_band/gen_report.py`）
