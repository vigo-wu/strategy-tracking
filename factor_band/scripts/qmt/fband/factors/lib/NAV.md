...
登记表：[../catalog.py](../catalog.py) 的 `LEAVES`（`registry` 按表取 `_factor_eval_<id>`）。作者改 `LEAVES` 的 `default`；运行时阈值是已写入的 `RECIPE.factor_params`，不写死在叶子里。不存在 `STOP_LOSS` 这类模块全局别名。`above_ma` 无阈值，不要给它写出空 `{}`。
...
| `above_ma` | [above_ma.py](above_ma.py) | 收盘 > 日线趋势均线 | `d_trend` | 无叶子阈值；窗是 `structure.ma.1d.trend`（`<=0` 关闸门） |
...
不要拆 `keltner_vol` 为 `keltner_inside & vol_shrink`。`above_ma` 无阈值，不要给它写出空 `{}`。
