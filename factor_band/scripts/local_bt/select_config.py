...
SELECT_SIDEBAR: dict[str, Any] = {
    ...
    "year_caption": "打分窗 %s–%s",
    # 仍给可能使用 max_from=year_max 的整数年控件（当前硬过滤已改为占比）
    "year_max_floor": 5,
    "filter_section": "硬过滤",
    "refresh_label": "刷新缓存",
    "research_caption": "扫描已有分年回测后打分；不自动改 `BOOK_STOCKS`。",
    "snippet_expander": "BOOK_STOCKS 草稿（勿写入 config）",
}
...
def _hlband_config_path(config_path: str | None = None) -> Path:
    ...
    return Path(__file__).resolve().parent.parent / "qmt" / "fband" / "config.py"
