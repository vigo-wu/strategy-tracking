...
启动:
  streamlit run factor_band/scripts/local_bt/app.py
  或: python factor_band/local_bt_ui.py
"""
...
from multiprocessing import current_process as _mp_current_process

_IS_MP_WORKER = _mp_current_process().name != "MainProcess"
...
if not _IS_MP_WORKER:
    ...
    try:
        ...
        _get_script_run_ctx(suppress_warning=True)
    except Exception:
        pass
    ...
    def _div_pack_for_compare(entry: dict) -> dict:
    ...
    def _render_one_div_detail(
        ...
        *,
        ...
        pack = _div_pack_for_compare(entry)
    ...
    def _render_div_results(
        ...
        *,
        ...
        if len(divs_ok) >= 2:
            ...
            budget = 50000.0
            for div in divs_ok:
                pack = _div_pack_for_compare(by_div[div])
    ...
    def _batch_div_packs(rows: list[dict], stock: str, year: str) -> dict:
            ...
            return {
                ...
                "by_div": {
                    DEFAULT_DIVIDEND_TYPE: {
                        ...
                        "sma": None,
                        "ema": None,
                        "result": cmp.get("ema") or cmp.get("sma") or cmp.get("result"),
                    }
                },
            }
    ...
    def _render_batch_run(
        ...
        *,
        ...
        if run_btn and picked and start_d and end_d and start_d <= end_d:
            ...
            try:
                ...
                if not payloads:
                    ...
                    write_typed_summaries(rows, split=split_mode)
                    all_rows = rows
                    bar.progress(1.0)
        ...
        show_div_in_label = (not picked_div) and len(unique_dividend_types(view_rows)) >= 2
        ...
        sibling = _batch_div_packs(all_rows, stock, year)
    ...
    def _bind_wf_progress(bar: Any, status: Any):
        ...
        def on_progress(ev: dict) -> None:
            try:
                total = int((ev or {}).get("total") or 0)
                done = int((ev or {}).get("done") or 0)
                if total > 0:
    ...
    if mode == GRID_MODE:
        ...
        if not labels:
            st.warning("未找到操作明细（`factor_band/回测记录` 或所选 `report/<type>/*操作明细*`）")
        else:
