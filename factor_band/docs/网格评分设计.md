...
**写死（侧栏不暴露）：** 盈亏盾满分 30、每负窗扣 15；结构因子占 70、笔数占 30（0 分线处 15 分、满分线处 30 分）；复原夏普 50 / 衰减 50；回撤用平方曲线；年化衰减 Hermite；触顶软乘 `DD_BREACH_MULT=0.2`；情况 C 封顶 `GEN_LUCK_CAP=80`；接近垫 `SCORE_CLOSE_PAD=1.0`。
...
任一同账户窗 `|max_dd| ≥ dd_cap` 时，加权总分再乘 `DD_BREACH_MULT=0.2`（软否决，非硬门）。
...
$$S_{\text{因子}} = \min\left(100,\ \max\left(0,\ \frac{\mathrm{Factor}}{\texttt{factor\_target}} \times 100\right)\right) \times 0.70$$

笔数：低于 `n_trades_floor` 得 0；`floor`–`full` 从 15 线性到 30；≥ `n_trades_full` 得 30。

### 3. 时空复原 \(S_{\mathrm{Res}}\)

- **夏普：** 调参/验收各自 `clip = min(1, max(0, 夏普 / sharpe_target))`，再 \(50 \times (clip_{\mathrm{tune}} + clip_{\mathrm{check}}) / 2\)。
- **年化衰减：** \(r =\) 验收几何年化 / 调参几何年化。任一窗年化为负、缺值、或调参年化 ≤0 得 0；\(r \ge 1\) 得 50；\(0 \le r < 1\) 用 Hermite：\(50 \times (3r^2 - 2r^3)\)（`r=0.5` 仍为 25）。

### 4. 空间泛化 \(S_{\mathrm{Gen}}\)

无空间隔离：`null`（摊权，见上）。有隔离但缺盲测窗：0。按调参 / 盲测全区间夏普四分支（`sharpe_target` 锚定情况 C 的绝对强度，**不是** `盲测 < target → 0` 硬门否决）：

- **A** 双正：\(S_{\mathrm{Gen}} = 100 \times \min(1,\ \text{盲测}/\text{调参})\)
- **B** 调参 >0 且盲测 ≤0：0
- **C** 调参 ≤0 且盲测 >0：\(\min(\texttt{GEN\_LUCK\_CAP},\ 50 \times \text{盲测}/\texttt{sharpe\_target})\)，封顶 80
- **D** 双 ≤0：0
...
选参流程与 CLI 见 `.cursor/skills/qmt-local-bt-grid/SKILL.md`。

## 七、实盘评估

参数锁定后的组合回放也用同一套 `score_cell`（`space_on` 恒为 True）。不改 `config.py`、不 deploy、不重跑组合 walk；改侧栏后再「只汇总」即可重裁。不删 `robust_gate.py`；旧 spec `gate` 残留可忽略。

整次 GO：**中位总分 ≥ `go_floor`（默认 50 分）**。无组可评分（`n_scored=0`）→ NO-GO。不把通过率 / 中位卡玛 / P10 回撤当否决；界面可展示中位四维与 P10 回撤。

窗口映射：

- `windows.all` = 调参年份 ∪ 验收年份（禁止 `year_start`–`check_end`，空年会稀释几何年化分母）
- `holdout_windows.all` 与 `holdout_windows.check` = 盲测 `deploy` KPI
- 防御仍用同账户 all/tune/check（盲测回撤不混入）；盈亏盾第三节点 = 盲测
- 结构 / 复原仍只用调参 vs 验收（盲测不进 `S_Str`）
- 泛化：双正用盲测 / 调参∪验收夏普线性比；盲测 ≤0 → 0；调参 ≤0 且盲测 >0 走情况 C（封顶 80）；缺盲测则 `S_Gen=0`，不摊权

笔数门槛是验收窗绝对笔数（默认 30），不再使用 `per_stock_year_min`。`go_floor` 由 `factor_band/scripts/local_bt/robust_score.py` 读写（网格 `score_cfg_for_json` 不带这个键）。现算依赖 summary 里的 `windows.all`；旧 run 缺该键时须先「只汇总」从成交表重算。

侧栏与网格相同的评分维旋钮，另加 **GO 中位分门槛**。组表用 pandas Styler 着色：回撤% ≤ `dd_cap×100`，笔数 ≥ `n_trades_floor`，夏普 / 卡玛 / 胜率% / 盈亏比对 0。

| 文件 | 职责 |
| :--- | :--- |
| `factor_band/scripts/local_bt/robust_score.py` | `go_floor` / `book_from_windows` / `eval_run_score` |
| `factor_band/scripts/local_bt/robust_summarize.py` | 写入 `windows.all` 与四维分；`summarize_run(..., score=)` |
| `factor_band/scripts/local_bt/robust_ui.py` | 侧栏评分维；主表现算；Styler |
| `.cursor/skills/qmt-local-bt-robust/SKILL.md` | 硬规则 3–5、检查清单 |
